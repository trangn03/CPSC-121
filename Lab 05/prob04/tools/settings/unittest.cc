...
TEST(StarTest, ConstructorPrintsStarName) {
  ...
  EXPECT_THAT(stream_out.str(), HasSubstr("Bellatrix was born"))
      << "    Star constructor should print out \"<star name> was born\" for the name given.";
}
...
TEST(StarTest, DestructorPrintsVolumeRatio) {
  ...
  EXPECT_THAT(stream_out.str(), HasSubstr("1.00 times"))
      << "    Star destructor should print out the relative volume of the star, "
...
TEST(StarTest, GetName) {
  Star star("Sun", 1);
  EXPECT_EQ(star.GetName(), "Sun") << "    GetName function should return the string passed in to the constructor.";
}

TEST(MainTest, ExecutesAndExits) {
  std::string user_input = "0\n";
  ASSERT_EXECEXIT("main", user_input, 3)
      << "      Your program should ask for the number of stars, and return immediately if 0 is entered.";
}

TEST(MainTest, ExecutesWithOneStar) {
  std::string user_input = "1\nVega\n2.362";
  ASSERT_EXECEXIT("main", user_input, 3)
      << "      Your program should ask for the number of stars, and for each star ask for the name and solar radius.";
}
...
TEST(MainTest, StarGoesOutOfScope) {
  ...
  ASSERT_DURATION_LE(3, {
    ASSERT_EXECTHAT("main", user_input, testing::MatchesRegex(
      ".*Betelgeuse was born.*Created star Betelgeuse.*786769.00 times.*"))
      << "    Your program should ask the user for the number of stars, and then "
      "use a loop ask the user for each star's name and size, then create a "
