...
TEST(CalculateAverage, NoElement) {
  ...
  ASSERT_DURATION_LE(3, {
    ASSERT_DOUBLE_EQ(CalculateAvg(unittest_vector), 0);
  })
}
...
TEST(CalculateAverage, OneElement) {
  ...
  ASSERT_DURATION_LE(3, {
    ASSERT_DOUBLE_EQ(CalculateAvg(unittest_vector), 1.5);
  })
}
...
TEST(CalculateAverage, MultipleElement) {
  ...
  unittest_vector[2] = 4.0;
    ASSERT_DURATION_LE(3, {
    ASSERT_DOUBLE_EQ(CalculateAvg(unittest_vector), 2.6666666666666665);
  })
}
...
TEST(CalculateAverage, MultipleElement2) {
  ...
  ASSERT_DURATION_LE(3, {
    ASSERT_DOUBLE_EQ(CalculateAvg(unittest_vector), 2.9);
  })
}
